...
1. **Analyze** -- Run `bin/docker-dev lint [target]` to identify offenses.
2. **Fix** -- Run `bin/docker-dev lint -a [target]` to apply safe auto-corrections. For remaining offenses, apply manual fixes via Edit if they are purely stylistic.
3. **Verify tests** -- Run `bin/docker-dev test` to confirm nothing broke. If tests fail, immediately revert with `git restore` and report the issue.
...
- RuboCop is the only linter installed. Don't run or suggest other linters or formatters unless the user adds them.
