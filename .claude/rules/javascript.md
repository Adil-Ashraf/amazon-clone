...
- Keep controllers small and single-purpose, like `dismissible_controller`
  ...
  `menu_controller` (disclosure menu with click-outside and Esc).
...
- Progressive enhancement: every feature must work as plain HTML first
  (forms submit, links navigate); Stimulus and Turbo only make it nicer. The
  header search is a normal GET form that works on Enter.
